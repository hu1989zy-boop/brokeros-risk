...
const caseNumber = process.env.E2E_Q018_CASE_NUMBER;
const decisionRef = process.env.E2E_Q018_DECISION_REF;
const actionRef = process.env.E2E_Q018_ACTION_REF;
...
test('associate decision → select current → associate action leaves resolve reachable', async ({
  page,
}) => {
  ...
  await runPreviewAction(page, 'Associate decision', 'Decision reference', decisionRef!, {
    Reason: 'Q-018 live decision association verification.',
  });
  version += 1;
  await expectVersion(page, version);

  await runSelectAction(page, 'Select current decision', 'Associated decision', decisionRef!, {
    Reason: 'Q-018 live decision selection verification.',
  });
  version += 1;
  await expectVersion(page, version);
  await expect(page.getByText(decisionRef!, { exact: true }).first()).toBeVisible();

  await runPreviewAction(page, 'Associate action', 'Action reference', actionRef!, {
    Reason: 'Q-018 live action association verification.',
  });
  version += 1;
  await expectVersion(page, version);
  await expect(page.getByText(actionRef!, { exact: true }).first()).toBeVisible();
  await expect(page.getByRole('button', { name: 'Resolve case' })).toBeEnabled();
});
...
async function loginAndOpenCase(page: Page, targetCaseNumber: string) {
  ...
  while ((await caseLink.count()) === 0) {
    ...
    if (await next.isDisabled()) {
      throw new Error('E2E_Q018_CASE_NUMBER was not found in the bounded Risk Case pages.');
    }
    await next.click();
  }
