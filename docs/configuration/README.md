...
| BrokerOS Q-016 React Console | `window.__BROKEROS_CONFIG__.apiBaseUrl` | `VITE_API_BASE_URL` (build-time fallback) | Absolute HTTP(S) URL | `http://localhost:8080` | Yes | web console | Internal | Frontend startup URL validation | replaceable `runtime-config.js`, Vite environment fallback | Browser reload | API endpoint only; never contains credentials |
| BrokerOS Q-016 React Console | `window.__BROKEROS_CONFIG__.oidcAuthority` | `VITE_OIDC_AUTHORITY` (build-time fallback) | Absolute OIDC issuer URL | `http://localhost:8180/realms/brokeros` | Yes | web console | Internal | Frontend startup URL validation; issuer contract remains server-verified | replaceable `runtime-config.js`, Vite environment fallback | Browser reload | Dev-to-production is an endpoint repoint; never contains credentials |
| BrokerOS Q-016 React Console | `window.__BROKEROS_CONFIG__.oidcClientId` | `VITE_OIDC_CLIENT_ID` (build-time fallback) | OIDC public client ID | `brokeros-risk-console` | Yes | web console | Public | Nonblank frontend startup validation | replaceable `runtime-config.js`, Vite environment fallback | Browser reload | Public client only; no client secret is supported |
| BrokerOS Q-016 React Console | `window.__BROKEROS_CONFIG__.oidcRedirectUri` | `VITE_OIDC_REDIRECT_URI` (build-time fallback) | Absolute browser URL | Current browser origin (`http://localhost:4173` in dev) | Yes | web console | Public | Frontend startup URL validation plus exact Keycloak allow-list | replaceable `runtime-config.js`, Vite environment fallback | Browser reload | Must match the deployment's exact approved Keycloak redirect URI |
...
- `dev` adds only the exact local React web origin used by the Q-016 CORS
